    ...
    def setup(self, ir: 'IR', aconf: Config):
        ...
        if cert_chain_file is not None:
            if not ir.file_checker(cert_chain_file):
                self.logger.error('cert_chain_file path {} does not exist'.format(cert_chain_file))
                return False
            cert_specified = True
        ...
        if private_key_file is not None:
            if not ir.file_checker(private_key_file):
                self.logger.error('private_key_file path {} does not exist'.format(private_key_file))
                return False
            cert_specified = True
        ...
        if (secret is not None) and (ir.tls_secret_resolver is not None):
            ...
            if resolved is None:
                self.logger.error("Secret {} could not be resolved".format(secret))
                return False
